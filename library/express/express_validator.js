...
var expressValidator = function (options) {
    ...
    validator.notIn = function () {
        var arg = _.object(['str', 'list', 'type'], Array.prototype.slice.call(arguments));
        var argName, str = arg.str.toLocaleLowerCase();
        for (var e in arg.list) {
            argName = arg.list[e].toLocaleLowerCase();
            if (arg.type === 'In' && str === argName) {
                return false;
            }
            if (arg.type === 'Contain' && str.indexOf(argName) > -1) {
                return false;
            }
        }
        return true;
